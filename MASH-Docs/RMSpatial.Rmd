...
#### Exposure and Infection in the Ross-Macdonald Model
...
#### Transmission to Mosquitoes Modified by Time at Risk

This formula does not, however, fully acknowledge the impact of mobility on transmission. Parasites also move when infectious people spending time here infect mosquitoes. Here, we derive the formulas as if mosquitoes don't move, and then make a correction about parasite dispersal in moving mosquitoes. A fully worked out model with mosquito movement will come later. 

Let $\vec \kappa$ denote the vector denoting the proportion of mosquitoes that become infected after feeding on a person here. In the Ross-Macdonald model, $\vec \kappa = c {\vec x_{(\Psi)}}$, where  ${\vec x_{(\Psi)}}$ is the total number of infected people at risk divided by the total population at risk, or 
$$
{\vec x_{(\Psi)}} = \frac{\Psi^T \times \vec X}{\Psi^T \times \vec H} = \frac{{\vec X_{(\Psi)}}}{{\vec H_{(\Psi)}}}
$$

Let $\vec M$, $\vec Y$, and $\vec Z$ denote the population density of mosquitoes, infected mosquitoes, and infectious mosquitoes, respectively. Similarly, let $\vec y = \vec Y/\vec M$ and $\vec z= \vec Z/\vec M$ denote the proportion of infectious mosquitoes. 

Let $a$ denote the human feeding rate. We model mosquito infection using the equation:


Not to complicate things too much, but mosquitoes do move around, from emergence to infection, and between the time they become infected and the time they become infectious. So really we need a model that fully tracks movement. For now, we will stop short of that and introduce, instead, a kernel describing dispersal of bites by infectious mosquitoes, $\Theta$, as a function of the formulas as computed above: 
$$
{\vec E_{(\Psi)}} =  \Theta \vec V \cdot \frac{\vec H}{\vec H_{(\Psi)}} \cdot  \frac{{c \vec x_{(\Psi)}}}{ \left(1+cS{\vec x_{(\Psi)}}\right)}
$$
