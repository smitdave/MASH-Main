...
# K=50
# offset = 0
# time = seq(from=1,to = 365,by = 0.5)
# signal = K*(1+sin(2*pi*(time-offset)/365))
# 
# noise = fArma::fgnSim(n = length(signal)+1,H = 0.9,method = "paxson")
# 
# lambda = signal + head(as.vector(noise),length(signal))
# 
# par(mfrow=c(2,1))
# plot(signal,type="l",ylim=c(0,250))
# plot(lambda,type="l",ylim=c(0,250))
# par(mfrow=c(1,1))
