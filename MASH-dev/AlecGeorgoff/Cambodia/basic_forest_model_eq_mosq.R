...
FM.ode = function(t,y,parms){with(as.list(c(y,parms)),{
  # Define equilibrium mosquito populations:
  Y_f = (a_f * c * V_f * X_f) / (H_f * g_f + a_f * c * X_f)
  Y_v = (a_v * c * V_v * ((1 - p) * X_f + X_v)) / (g_v * (H_f * (1 - p) + H_v) + a_v * c * ((1 - p) * X_f + X_v))
...
parms = c(b=b, r=r, c=c, n=n,
          a_f=a_f, g_f=g_f, H_f=H_f, X_f=X_f, p=p, V_f=V_f,
          a_v=a_v, g_v=g_v, H_v=H_v, X_v=X_v, V_v=V_v)

inits = c(X_f=X_f0, X_v=X_v0)
