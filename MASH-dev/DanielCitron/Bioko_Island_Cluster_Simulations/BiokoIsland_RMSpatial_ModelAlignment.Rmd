...
We also need to re-scale the number of trips according to the population in each patch.  Only `n` people in each patch are surveyed over the course of 3 years, so we should rescale according to `pop/(n/3)`.  Similarly, we want to know the number of times that someone might travel over the full year, not just 8 weeks - so we again rescale by 52/(8*3) - the extra factor of 3 is added because we include 3 years' worth of respondent time.
```{r}
# Replace all of the n=NA with 0
travel.model.data[is.na(n)]$n <- 1

# Rescale the 3 years of survey respondents' number of times traveled by the number of people living in each patch
travel.model.data$Freq.all <- round(travel.model.data$Freq*travel.model.data$pop/travel.model.data$n)

# Rescale according to a year's worth of time, instead of just 8 weeks
# But we also need to account for 3 years' worth of respondent time
#travel.model.data$Freq.all <- round(travel.model.data$Freq.all*52/(8*3))
# Alternatively, do this below
```
...
gravity.zinb.log.w <- zeroinfl(Freq.all ~ pop.log + pop.Region.log + log(TravelTime.Weighted), 
                   data = travel.model.data, dist = "negbin")

gravity.zinb.log.s <- zeroinfl(Freq.all ~ pop.log + pop.Region.log + log(TravelTime.Sum), 
                               data = travel.model.data, dist = "negbin")
```
...
holder <- zeroinfl(Freq.all ~ pop + pop.Region + TravelTime.Weighted, 
                   data = travel.model.data, dist = "negbin")
AIC(holder)

# non-log-transformed travel times
gravity.zinb.log.w <- zeroinfl(Freq.all ~ pop.log + pop.Region.log + TravelTime.Weighted, 
                               data = travel.model.data, dist = "negbin")
AIC(holder)

# zero-inflated poisson fit
holder <- zeroinfl(Freq.all ~ pop.log + pop.Region.log + TravelTime.Weighted, 
                               data = travel.model.data, dist = "poisson")
AIC(holder)

# ordinary negative binomial fit
holder <- glm.nb(Freq.all ~ pop.log + pop.Region.log + TravelTime.Weighted,
                 data = travel.model.data)
AIC(holder)

# ordinary negative binomial fit, without log transformed variables
holder <- glm.nb(Freq.all ~ pop + pop.Region + TravelTime.Weighted,
                 data = travel.model.data)
AIC(holder)

# ordinary poisson fit
holder <- glm(Freq.all ~ pop.log + pop.Region.log + TravelTime.Weighted, family = "poisson", 
              data = travel.model.data)
AIC(holder)
```
...
for (i in 1:194){
  ...
  d <- travel.model.data[areaId==areaIds[i]]$pop
  holder <- holder/d
