...
$$
\begin{split}
  & \frac{d X_i}{dt} = b_i \left( \psi \cdot \vec{E}_{(\psi)} \right)_i \times \left(H_i - X_i\right) - r X_i \\
...
$$
The second term $\vec{E}_{(\psi)}$ represents the force of infection (roughly, EIR) on humans, properly normalized to account for the fact that humans are moving around and visiting different patches:

$$
 \left(\vec{E}_{(\psi)}\right)_i \equiv a_i \frac{Z_i}{\left(\vec{H}_{(\psi)}\right)_i} = a_i \frac{Z_i}{\left( \psi^T \cdot \vec{H}\right)_i}
$$
...
$$
\begin{split}
  & 0 = b_i \left( \psi \cdot \vec{E}_{(\psi)} \right)_i \times \left(H_i - X_i\right) - r X_i \\
  & \frac{r}{b_i}\frac{X_i}{H_i - X_i} = \left( \psi \cdot \vec{E}_{(\psi)} \right)_i \\
  & \Rightarrow \left(\vec{E}_{(\psi)}\right)_i = \sum_i \left(\psi^{-1}\right)_{ij} \cdot \left(\frac{r}{b_i}\frac{x_i}{1 - x_i}\right) \\
\end{split}
$$
...
$$ 
\left(\vec{E}_{(\psi)}\right)_i = \sum_i \left(\psi^{-1}\right)_{ij} \cdot \left(\frac{r}{b_i(1 - \rho)} \frac{x_i}{\left(1 - (1 + \frac{\rho}{\epsilon}\frac{r}{1-\rho} ) x_i \right)}\right)
$$
...
```{r}
g <- r/b/(1-rho)*pfpr.input/(1-(1+rho*r/eta/(1-rho))*pfpr.input)
FOI <- ginv(P.ij) %*% g
...
One important constraint on the solution here is that the FOI must be non-negative.

__Sanity Check__ does the mean EIR calculated across all patches in each region come close to matching the EIR calculated for each region?
...
pr <- p2 + geom_polygon(data = plot.data, aes(x = long, y = lat.x, group = group, fill = pfpr*100), color = NA, size = 0.25) +
  scale_fill_gradient(name="PfPR (%)", low="yellow", high="red", limits=c(0,50)) +
...
eir <- p2 + geom_polygon(data = plot.data, aes(x = long, y = lat.x, group = group, fill = foi*365), color = NA, size = 0.25) +
  scale_fill_gradient(name="FOI (Annual EIR)", low="yellow", high="red", limits=c(0,3)) +
  geom_polygon(data = bioko, aes(x = long, y = lat, group = group), color = "black", fill=NA, size = 0.25) +
  theme(axis.line=element_blank(),axis.text.x=element_blank(), axis.text.y=element_blank(),axis.ticks=element_blank(),
        axis.title.x=element_blank(),
        axis.title.y=element_blank(), panel.background=element_blank(), legend.position=c(0.2, 0.8))
