...
# output directory
directory =  "BI_Macro_Calibration/patch_2083_timeaway/"#"BI_Macro_Calibration/patch_273_toomuchtravel/"#"BI_Macro_Calibration/patch_582_toomuchtravel/" #
...
####
# EIR etc with TOO MUCH travel? ----
####
areaIds = sort(travel.model.data$areaId)
patch_areaId = 2083 #273 #582 #
patch_ix = which(areaIds == patch_areaId)

travel.model.data[areaId == patch_areaId]$prall_map
travel.model.data[areaId == patch_areaId]$freq.model.fit


### Calculate TaR matrix
TaR.1 <- diag(1, nrow = 194+7, ncol = 194+7)
freq.model.fit <- travel.model.data$freq.model.fit
freq.model.fit[patch_ix] <- freq.model.fit[patch_ix]*10
freq.model.fit[patch_ix]
for (i in 1:194){
  holder <- c(0,0,0,10/(freq.model.fit[i]^(-1) + 10),0,0,0)
  TaR.1[i, (194 + 1):(194 + 7)] <- as.vector(holder)
  TaR.1[i,i] <- 1 - sum(holder)
}
TaR.1[patch_ix, c(patch_ix, 195:201)]

# and now calculate FOI
odds.vector <- r/(1-rho)*reservoir.data$pfpr.input/(1-(1+rho*r/eta/(1-rho))*reservoir.data$pfpr.input)
h.tt <- ginv(TaR.1) %*% as.vector(odds.vector)
h.tt[patch_ix] # local h
travel.model.data[areaId == patch_areaId]$h.1

lambda.tt <- with(travel.model.data[areaId == patch_areaId], (1-p)/p*h.tt[patch_ix]*(pop*TaR.1[patch_ix, patch_ix])/a/b/z.1) #with(travel.model.data[areaId == patch_areaId], (1-p)/p*h.tt[patch_ix]*pop/a/b/z.1)
travel.model.data[areaId == patch_areaId]$lambda.1
m.tt <- with(travel.model.data[areaId == patch_areaId], lambda.tt/pop/TaR.1[patch_ix, patch_ix]*p/(1-p))
travel.model.data[areaId == patch_areaId]$m.1
...
patch.lambda <- c(lambda.tt, rep(0,7))
...
moveMat <- diag(n)
moveMat[1,] <- with(travel.model.data[areaId==areaIds[patch_ix]], c(0,0,0,0,1,0,0,0)) #c(0, p.off, p.ban, p.lub, p.mal, p.mok, p.ria, p.ure))
#rowSums(moveMat)
# Patch Parameters ####
...
Z <- c(eir[1]/a*(travel.model.data[areaId==patch_areaId]$pop*TaR.1[patch_ix, patch_ix]), rep(0, 7) ) #c(Z.1[patch_ix], rep(0, 7) )
#m.tt*a*travel.model.data[areaId==patch_areaId]$z.1
#a*Z[1]/n_humans
...
humanPar = lapply(X = 1:n_humans,function(i){
  list(
    ...
    tripDuration = c(0,0,0,0,10,0,0,0),# this is if we use TaR matrix 1
    # tripDuration = c(travel.model.data$to.time.e[patch_ix],3,3,3,3,3,3,3,3,3,3), # This is if we use TaR matrix 2
    tripFrequency = freq.model.fit[patch_ix]#travel.model.data$freq.model.fit[patch_ix]
  )
})
...
t4 <- SIP.Conversion.Curves(human.pathogen.path, human.move.path, patch.human.populations, 365)
ggplot(data = t4) +
  ...
  xlim(0,365) + ylim(0,300) +
  xlab("Time") + ylab("N")

#View(t4)


mean(t4[location == 5 & time > 200, sum(N), by = c("time", "location") ]$V1)/n_humans
mean(t4[location == 1 & time > 200, sum(N), by = c("time", "location") ]$V1)/n_humans


mean((t4[time > 200 & status=="I", sum(N), by = "time"]$V1)/n_humans)
...
#directory = "BI_Macro_Calibration/patch_273_toomuchtravel/"#"BI_Macro_Calibration/patch_582/"#"BI_Macro_Calibration/patch_273/"#
...
m <- matrix(NA, ncol = 10, nrow = 3*n*5*365)
for (i in 1:10){
...
#View(h[time > 1000 & status == "I"][order(time)])
...
h$sum.m <- rowSums(m)/10
occupy <- dcast(h[time > 1000, sum(sum.m), by = c("time", "location")], time ~ location)
#occupy <- dcast(h[time > 1000, sum(N), by = c("time", "location")], time ~ location)
...
TaR.1[patch_ix, c(patch_ix, 195:201)] %*% h.tt[c(patch_ix, 195:201)]
occupancy %*% h.tt[c(patch_ix, 195:201)]

# This is the right way to parameterize time spent in different locations
c(1/freq.model.fit[patch_ix]/(10 + 1/freq.model.fit[patch_ix]), 10/(10 + 1/freq.model.fit[patch_ix]))
#c(1/freq.model.fit[patch_ix]/(3 + 1/freq.model.fit[patch_ix]), 3/(3 + 1/freq.model.fit[patch_ix]))


### Rewrite the TaR matrix according to how we are ACTUALLY parameterizing the above simulation:
TaR.1[patch_ix, c(patch_ix, c(195:201))] %*% h.tt[c(patch_ix, c(195:201))]

# Compare the above quantity to the actual prevalence:
h[status == "I" & time > 1000][, sum(N.means), by = c("time", "location")]
pr.actual <- mean(h[status == "I" & time > 1000][, sum(N.means), by = c("time")]$V1)/n_humans
pr.actual

psi.h <- TaR.1[patch_ix, c(patch_ix, c(195:201))] %*% h.tt[c(patch_ix, c(195:201))]
...
### Home times
move0 <- fread("BI_Macro_Calibration/patch_2083_toomuchtravel/HumanMove_Run0.csv")
move0[order(humanID)][humanID==1]
...
#trip.times <- c()
#for (human.ix in 1:n_humans){
#  trip.times <- c(trip.times, move0[order(humanID)][humanID==human.ix][(which(move0[order(humanID)][humanID==human.ix]$location == 2) + 1)]$time - move0[order(humanID)][humanID==human.ix][which(move0[order(humanID)][humanID==human.ix]$location == 2)]$time)
#}
#summary(trip.times)
...
for (file in human.movement.outputs){
  move0 <- fread(paste0(directory, file))
  for (human.ix in 1:n_humans){
    trip.times <- c(trip.times, move0[order(humanID)][humanID==human.ix][(which(move0[order(humanID)][humanID==human.ix]$location == 5) + 1)]$time - move0[order(humanID)][humanID==human.ix][which(move0[order(humanID)][humanID==human.ix]$location == 5)]$time)
  }
}
summary(trip.times)
hist(trip.times)
...
move0[order(humanID)][humanID==1]
move0[order(humanID)][humanID==1][(which(move0[order(humanID)][humanID==1]$location == 2))]

mean(move0[order(humanID)][humanID==1][(which(move0[order(humanID)][humanID==1]$location == 5) + 1)]$time - move0[order(humanID)][humanID==1][which(move0[order(humanID)][humanID==1]$location == 5)]$time)
mean(move0[order(humanID)][humanID==2][(which(move0[order(humanID)][humanID==2]$location == 2) + 1)]$time - move0[order(humanID)][humanID==2][which(move0[order(humanID)][humanID==2]$location == 2)]$time)


move0[order(humanID)][humanID==1][sort(c(which(move0[order(humanID)][humanID==1]$location == 2),
                                         (which(move0[order(humanID)][humanID==1]$location == 2) + 1)))]

move0[order(humanID)][humanID==1][(which(move0[order(humanID)][humanID==1]$location == 2) + 1)]$time -
  move0[order(humanID)][humanID==1][which(move0[order(humanID)][humanID==1]$location == 2)]$time


move0[order(humanID)][humanID==1 & location == 5][seq(2,36,2)]
move0[order(humanID)][humanID==1 & location == 5][seq(1,35,2)]

move0[order(humanID)][humanID==1 & location == 5][seq(2,36,2)]$time-move0[order(humanID)][humanID==1 & location == 5][seq(1,35,2)]$time
move0[order(humanID)][humanID==1 & location == 2]

mean(move0[order(humanID)][humanID==1][seq(2,136,2)]$time - move0[order(humanID)][humanID==1][seq(1,135,2)]$time)

### Probability of going different places
hist(move0[event == "takeTrip"]$location)
moveMat[1,]
...
for (file in human.movement.outputs){
  move0 <- fread(paste0(directory, file))
  trip.types <- c(trip.types, move0[event == "takeTrip"]$location)
}
trip.types.hist <- hist(trip.types,breaks = seq(1.5,7.5,1))
trip.types.hist$counts
ntrips <- length(trip.types)
trip.types.hist$counts/ntrips
moveMat[1,2:8]
