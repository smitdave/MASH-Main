...
In computing the outcome of a flight bout, variables track the mosquito's current state and the time when it started the bout, and the mosquito's next state and the time when it will start the next bout. The start of a bout, is \verb1tNext1, a variable that was checked to be sure other elements of the simulation it depends on have already been correctly set. Upon entering the bout, we update the mosquitoes time, state, and location: 
\begin{itemize}
...
%
\item The function  \verb1restingSpot()1 is then called. A mosquito can fail to find a suitable resting place and thus decided to leave (denoted \verb1bfrs.f1) in which case its behavioral state is set to {\bf F}.
%
