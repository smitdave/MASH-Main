...
\begin{itemize} 
  ...
  \item Search or Attempt. The single letter name usually refers to the attempt, but the blood feeding search is denoted [F=Bs] and the egg laying search is [L=Os] (mainly for the MDETES equations). The other search states have not been given a single letter name, they are called [Ms], [Ss], or [Es]. 
\end{itemize}
\item Location. 
\begin{itemize} 
...
\item[\SSbout] :: Sugar feeding search bout
\item[\Sbout] :: Sugar feeding attempt 
\item[\MSbout] :: Mating search bout
\item[\Mbout] :: Mating attempt bout
\item[\ESbout] :: Estivation search bout 
\item[\Ebout] :: Estivation bout, in which a mosquito becomes active
\end{itemize}
%
...
Some bouts are required, and others are optional. The core bouts are \Fbout, \Bbout, \Rbout, \Lbout, and \Obout\; must all be in place to run MBITES at all, but the optional states \Sbout, \Mbout, and \Ebout\; can be turned on and utilized, if it suits the problem at hand. \demo\; uses all the states, except for estivation.  In \minmod, only the core bouts are used; the others must be turned on and configured. 
