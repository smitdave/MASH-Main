...
# tsim_foi <- sapply(X = 1:N,FUN = function(i){
#   bites <- rnbinom(n = 1260,size = tdat_gg$size[[i]],prob = tdat_gg$prob[[i]])
# })

b <- 0.55

aEIR_t <- colSums(tsim_gg)/(1260/365)
aEIR_k <- colSums(ksim_gg)/(1260/365)
aEIR_j <- colSums(jsim_gg)/(1260/365)

aeff_t <- rbinom(n = length(aEIR_t),size = floor(aEIR_t),prob = rep(b,length(aEIR_t)))
aeff_k <- rbinom(n = length(aEIR_k),size = floor(aEIR_k),prob = rep(b,length(aEIR_k)))
aeff_j <- rbinom(n = length(aEIR_j),size = floor(aEIR_j),prob = rep(b,length(aEIR_j)))

ineff <- data.frame(
  EIR = c(aEIR_t,aEIR_k,aEIR_j),
  FOI = c(aeff_t,aeff_k,aeff_j),
  site = rep(c("Tororo","Kanungu","Jinja"),each=length(aEIR_t))
)

ineff$aeff <- ineff$EIR / ineff$FOI

ggplot(data = ineff) +
  geom_point(aes(x=EIR,y=FOI,color=site),alpha=0.05) +
