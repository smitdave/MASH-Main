...
\item Power Laws
  \begin{itemize}
  \item For mosquito counts
  \item For efficient transmission
  \end{itemize}
\item Decomposing Heterogeneity: That we can accurately model the process. 
  \begin{itemize}
  \item Temporal
  \item Household biting propensities
  \item Residual error ratios
  \item Environmental stochasticity
     \begin{itemize}
     \item Fitting $\xi$, $k$ 
     \item Fitting $\xi=g(\mu)$, $k=f(\mu)$. 
     \end{itemize}
  \end{itemize}
\item Simulated Power Laws for Counts: That the model faithfully reproduces the power laws for all the counts. (what is our measure of the GoF?)
  \begin{itemize}
  \item Counts by Month
    \begin{itemize}
    \item All Anopheles
    \item Infectious
    \end{itemize}
  \item Counts by Household
    \begin{itemize}
    \item All Anopheles
    \item Infectious
    \end{itemize}
  \end{itemize}
\item Simulated Power Laws for Exposure and Infection: 
\item Final version of figures
\end{enumerate}
...
The following objects hold the data, read from an input file:
 
<<echo=TRUE>>=
tororo = read.table("TOROROnew.txt", header=T)
jinja = read.table ("JINJAnew.txt", header=T)
kanungu = read.table ("KANUNGUnew.txt", header=T)
names(tororo) 

@ 

This document describes analysis after the analysis to fit the
temporal signal and household biting weights.  Each line in the
file describes one sample:  the count ($M$, \verb1obs1), the
household ID ($h$, \verb1hhid1), the day of the study
($d$,counting from 1, \verb1day1, the month (\verb1monthyear1),
the household biting weight ($\omega_i$, in \verb1w1), the daily
expectation $S_d$ \verb1Sd1,  the residual error ratio ($E$,
where $ES\omega = M$), the number of mosquitoes tested and
positive and the number not sampled (\verb1ntested1, \verb1npos1,
\verb1npos1), the sporozoite rate for the sample (\verb1SR1), the
EIR found by multipliying the count by the SR (\verb1EIR1), and a
simulated EIR found by adding more infectious mosquitoes than
observed by drawing a binomial with the number of unsampled
mosquitoes and the sporozoite rate for the sample (\verb1sEIR1).  
...
\subsection{Power Laws for Mosquito Counts Data}

We write functions to pull out all the observations for each
month or household:
...
Functions are defined that plot the power law relations by month and
year (set \verb1echo=TRUE1 to add them to the pdf).  
...
\subsection{The Epidemiological Power Laws}

<<>>=
load("data_plot_dave.Rdata")
@
...
\section{Decomposing the Heterogeneity}

We create some objects that are convenient for plotting:
\verb1Xd1 has the list of all days when samples occurred, for
$X\in\left\{J,K,T\right\}$.  
...
We create some objects that are convenient for plotting:
\verb1XSd1 for $X\in\left\{J,K,T\right\},$  has the list of all
the corresponding $S_d$ values.  
...
We also create a smoothed time series of the sporozoite rate for
each day, called \verb1Xz1:
...
<<echo=FALSE, fig=TRUE, width = 6.5, height=4>>=
...
\subsection{Household Biting Weights}
...
\subsection{Residual Error Ratios}
...
<<echo=FALSE, fig=TRUE, width=5, height=9.5>>=
...
What is of scientific interest here is that these data from Uganda replicate pattern observed elsewhere -- that as the intensity of exposure increases, the ratio of infectious bites to infections shrinks: 
...
\section{Fitting, Using Likelihood}


One way to parameterize the negative binomial is with $\mu$ and $k$: 
$$\mbox{dnbinom}(\mu = S_{d} \omega_i, size= k = f_l(\mu))$$
which arises from the $\Gamma$-Poisson mixture process where the
mean of the Poisson is $$\mbox{dgamma}(shape = k, scale=\mu/k).$$ 
To show these two distributions are equivalent, the following is a simple check:
%
<<fig=TRUE>>=
set.seed(22)
mu = 10; k=.2; N=10000
...
@
An alternative parameterization is with $\mu$ and $\xi$: 
$$\mbox{dnbinom}(size = \mu g_l(\mu), prob =
\frac{g_l(\mu)}{1+g_l(\mu)})$$ where the mean of the Poisson is 
$$\mbox{dgamma}(shape = \mu \xi = \mu g_l(\mu), scale=1/\xi = 1/g_l(\mu)).$$ 

The two parameterizations are equivalent under the the assumption
that $\xi \mu = g_l(\mu) \mu = f_l(\mu) = k$. 
...
We fit a single parameter to the variance at each site, first
using the $\xi$ parameterization: 
...
and next with the $k$ parameterization
...
\subsection{Quantiles Analysis}

\paragraph{MLE Fits, by $\xi$}

The following does the fitting.  Note that since we're most interested in the relationship when the expected values of $\mu$ are high, we 
exclude low values of $\mu$. 
...
<<echo=FALSE, fig=TRUE, height=10, width=7>>=
par(mfrow=c(3,1))
...
\subsection{Total Anopheles, By Month} 

<<echo=FALSE, fig=TRUE, width=6.5, height=6.5>>=
...
\subsection{Total Anopheles, By Household} 
...
\subsection{Infectious Anopheles, By Month} 
...
\subsection{Infectious Anopheles, By Hzousehold} 
...
\section{Exposure and Infection}
...
<<echo=FALSE, fig=TRUE, width=9, height=9.5>>=
  source ("Figure1.R") 
...
<<echo=FALSE, fig=TRUE, height=6, width=12>>=
source ("Figure3.R")
pdf("Figure3.pdf", height=8, width=12)
source ("Figure3.R")
dev.off(dev.cur())
@
