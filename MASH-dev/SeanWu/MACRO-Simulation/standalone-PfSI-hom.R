...
simout_t <- foreach(i = 1:nrep, .combine="rbind",.options.snow=opts) %dopar% {
  ...
  for(t in 1:(length(blocks)-1)){
    ...
    h_hatS[t] <- sum(simout$foi[block_t,])/DELTAT/N_S

    # attack rate
    if(DELTAT == 1){
      AR[t] <- sum(simout$ar[block_t,S_ix] > 0) / N_S
    } else {
...
simout$aeff[simout$aeff==0] <- NA

saveRDS(object = simout,file = here::here("sim/PfSI_hom.rds"))
...
#   guides(colour = FALSE) +
#   scale_y_log10(breaks = c(1,2.2, 4.4, 9.5),labels = c("1:1","1.7:1","2.7:1","7.4:1")) +
...
simout_t <- tiny_pfsi(tmax = 1260,nh = N,init = sample(x = c("S","I"),size = N,replace = TRUE,prob = c(1-pfpr_means[["Tororo"]],pfpr_means[["Tororo"]])),
                      EIR_size = replicate(N,TTSd*TTz,simplify=FALSE),EIR_prob = replicate(N,rep(0,1260),simplify=FALSE),pb = TRUE)

simout_k <- tiny_pfsi(tmax = 1260,nh = N,init = sample(x = c("S","I"),size = N,replace = TRUE,prob = c(1-pfpr_means[["Kanungu"]],pfpr_means[["Kanungu"]])),
                      EIR_size = replicate(N,KKSd*KKz,simplify=FALSE),EIR_prob = replicate(N,rep(0,1260),simplify=FALSE),pb = TRUE)

simout_j <- tiny_pfsi(tmax = 1260,nh = N,init = sample(x = c("S","I"),size = N,replace = TRUE,prob = c(1-pfpr_means[["Jinja"]],pfpr_means[["Jinja"]])),
                      EIR_size = replicate(N,JJSd*JJz,simplify=FALSE),EIR_prob = replicate(N,rep(0,1260),simplify=FALSE),pb = TRUE)
...
saveRDS(object = ineff_df,file = here::here("sim/PfSI_hom_ineff.rds"))
