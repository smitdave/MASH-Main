...
/* assign a key-value pair */
template <>
void parameters::set_param<int>(const std::string &key, const int val){
  paramsI.insert(std::make_pair(key,val));
};

template <>
void parameters::set_param<double>(const std::string &key, const double val){
  paramsD.insert(std::make_pair(key,val));
};

/* get a value by key */
template <>
double parameters::get_param<double>(const std::string &key){
  return paramsD.at(key);
}

template <>
int parameters::get_param<int>(const std::string &key){
