...
plot(distBins,sitesKern[[1]]$PDF_sth$y,type="l",col=adjustcolor("mediumblue",0.05),lwd=3,ylim=c(.Machine$double.eps,maxy),xlim=c(0,5))
for(i in 2:nsite){
  lines(distBins,sitesKern[[i]]$PDF_sth$y,col=adjustcolor("mediumblue",0.05),lwd=3)
}
lines(distBins,meanPDF,col=grey(level = 0.1),lwd=3)

# plot smoothed CDFs
plot(sitesKern[[1]]$CDF_sth$x,sitesKern[[1]]$CDF_sth$y,type="l",col=adjustcolor("firebrick3",0.05),lwd=3,ylim=c(0,1),xlim=c(0,5))
for(i in 2:nsite){
  lines(sitesKern[[i]]$CDF_sth$x,sitesKern[[i]]$CDF_sth$y,col=adjustcolor("firebrick3",0.05),lwd=3)
}
