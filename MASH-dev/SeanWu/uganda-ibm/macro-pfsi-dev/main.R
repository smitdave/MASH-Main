...
library(tidyverse)
pfsi <- readr::read_csv(here::here("/output/pfsi_1.csv"))

pfsi <- pfsi %>% gather(key, value, 3:10)
pfsi <- pfsi %>% filter(time > 0)
pfsi <- pfsi %>% filter(key != "incidence_resident")
pfsi <- pfsi %>% filter(key != "incidence_traveller")
