// Generated by using Rcpp::compileAttributes() -> do not edit by hand
// Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

#include <RcppArmadillo.h>
#include <Rcpp.h>

using namespace Rcpp;

// run_macro
void run_macro(const int tmax, const Rcpp::List& human_pars, const Rcpp::List& mosquito_pars, const Rcpp::List& patch_pars, const Rcpp::List& model_pars, const Rcpp::List& log_streams, const Rcpp::List& vaxx_events, const bool verbose);
RcppExport SEXP _macro_pfsi_rm_run_macro(SEXP tmaxSEXP, SEXP human_parsSEXP, SEXP mosquito_parsSEXP, SEXP patch_parsSEXP, SEXP model_parsSEXP, SEXP log_streamsSEXP, SEXP vaxx_eventsSEXP, SEXP verboseSEXP) {
BEGIN_RCPP
    Rcpp::RNGScope rcpp_rngScope_gen;
    Rcpp::traits::input_parameter< const int >::type tmax(tmaxSEXP);
    Rcpp::traits::input_parameter< const Rcpp::List& >::type human_pars(human_parsSEXP);
    Rcpp::traits::input_parameter< const Rcpp::List& >::type mosquito_pars(mosquito_parsSEXP);
    Rcpp::traits::input_parameter< const Rcpp::List& >::type patch_pars(patch_parsSEXP);
    Rcpp::traits::input_parameter< const Rcpp::List& >::type model_pars(model_parsSEXP);
    Rcpp::traits::input_parameter< const Rcpp::List& >::type log_streams(log_streamsSEXP);
    Rcpp::traits::input_parameter< const Rcpp::List& >::type vaxx_events(vaxx_eventsSEXP);
    Rcpp::traits::input_parameter< const bool >::type verbose(verboseSEXP);
    run_macro(tmax, human_pars, mosquito_pars, patch_pars, model_pars, log_streams, vaxx_events, verbose);
    return R_NilValue;
END_RCPP
}

static const R_CallMethodDef CallEntries[] = {
    {"_macro_pfsi_rm_run_macro", (DL_FUNC) &_macro_pfsi_rm_run_macro, 8},
    {NULL, NULL, 0}
};

RcppExport void R_init_macro_pfsi_rm(DllInfo *dll) {
    R_registerRoutines(dll, NULL, CallEntries, NULL, NULL);
    R_useDynamicSymbols(dll, FALSE);
}
